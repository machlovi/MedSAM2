...
class DualLogger:
    def __init__(self, base_logger, wandb_run=None, rank=0):
        ...
        self.rank = rank
        # Use single global step counter to avoid conflicts
        self.global_step = 0
    ...
    def log(self, key: str, value, step: int, phase: str = None):
        ...
        if self.wandb is not None and self.rank == 0:
            ...
            # Use the provided step directly - don't modify it
            self.wandb.log({wandb_key: value}, step=step)
...
class Trainer:
        ...
        while self.epoch < self.max_epochs:
            ...
            if self.is_intermediate_val_epoch(self.epoch):
                ...
                if current_val_loss is not None:
                    ...
                    self.logger.log("early_stop/patience", patience_counter, self.epoch, phase="train")
                    self.logger.log("early_stop/best_val_loss", best_val_loss, self.epoch, phase="train")
    ...
    def run_val(self):
        ...
        # Log validation metrics with epoch as step
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            try:
                ...
                if data_iter % self.logging_conf.log_scalar_frequency == 0:
                    for j, param_group in enumerate(self.optim.optimizer.param_groups):
                        for option in self.optim.schedulers[j]:
                            optim_prefix = (
                                f"{j}_" if len(self.optim.optimizer.param_groups) > 1 else ""
                            )
                            self.logger.log(
                ...
                self.global_step += 1
                self.steps[phase] = self.global_step
    ...
    def _log_timers(self, phase):
        ...
        step_to_use = self.global_step if phase == Phase.TRAIN else self.steps[phase]
        self.logger.log(
    ...
    def _setup_components(self):
        ...
        if get_rank() == 0:
            ...
            # Define metrics with proper step counters
            wandb.define_metric("global_step")
            wandb.define_metric("epoch")
            
            # Training metrics use global_step
            wandb.define_metric("train/*", step_metric="global_step")
            # Validation metrics use epoch
